    ...
    if VER_MOV_STEP != 0 and HOR_MOV_STEP != 0:
        ...
        for ver in range(VER_MOV_STEP):
            for hor in range(HOR_MOV_STEP):
                ...
                # convert image with same hue value
                img_arr = uniform_hue(img_arr, 100, img_size)
...
if __name__ == '__main__':
    ...
    def mp_func(path, gpu_id):
        ...
        # ==========global NMS============
        # # step1: get no-global NMS list
        no_global_keep_idx = []
        for i in range(len(PREDICTIONS)):
            if GLOBAL_NMS_LIST[PREDICTIONS[i]['class']] == 0:
                no_global_keep_idx.append(i)
        
        # step2: get global NMS list          
        global_keep_idx = []
        dets3 = [[d['x'], d['y'], d['x']+d['w']-1, d['y']+d['h']-1, d['p']] for d in PREDICTIONS] # dets3 format: [[xmin,ymin,xmax,ymax,p], [], ...]  
        keep3 = py_cpu_nms(np.array(dets3), min(list(FINAL_NMS_THRESH_DICT.values())))        
        
        for keep_idx in keep3:
            if GLOBAL_NMS_LIST[PREDICTIONS[keep_idx]['class']] == 1:
                global_keep_idx.append(keep_idx)
        
        # step3: concatenate keep_idx
        final_keep = no_global_keep_idx + global_keep_idx

        # step4: resort prediction result   
        PREDICTIONS = [PREDICTIONS[idx] for idx in final_keep]
        #===============================
